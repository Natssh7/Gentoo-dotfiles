...
if [[ "$1" = "--bad-apple" ]]; then
	...
	cp scripts/color_generation/specials/_material_badapple-l.scss $STATE_DIR/scss/_material.scss
	colornames=$(cat scripts/color_generation/specials/_material_badapple-l.scss | cut -d: -f1)
	colorstrings=$(cat scripts/color_generation/specials/_material_badapple-l.scss | cut -d: -f2 | cut -d ' ' -f2 | cut -d ";" -f1)
